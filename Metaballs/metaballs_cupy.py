""" Pygame application which calculate metaballs with cuda
"""
...
def euclid_dist(vector_p1, vector_p2):
    ...
    return cp.hypot(distances[:, :, 0], distances[:, :, 1])


def gray(im, mode="saturate"):
    """ ceate grayscale image from 2d array """
    # im = 255 * (im / im.max())
    if mode == "saturate":
        im[im >= 255] = 255
    else:
        im = 255 * (im / im.max())
    w, h = im.shape
    ret = cp.empty((w, h, 3), dtype=cp.uint8)
...
class MetaBalls:
    ...
    def __init__(self) -> None:
        ...
        self._grid_xy = np.zeros(
            (SCREEN_WIDTH * SCREEN_HEIGHT * 2), dtype="int32"
        ).reshape(SCREEN_WIDTH, SCREEN_HEIGHT, 2)

        self._img = cp.zeros(SCREEN_WIDTH * SCREEN_HEIGHT).reshape(
        ...
        self._grid_xy = cp.array(self._grid_xy)
    ...
    def draw(self) -> None:
        ...
        for ball in self._balls:
            distances = euclid_dist(self._grid_xy, ball.pos_cp)
            self._img += ball.radius / distances

        pygame.surfarray.blit_array(self._screen, np.array(gray(self._img).get()))
